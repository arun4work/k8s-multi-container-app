...
2. **`build-and-push`** *(only on push to `master`, gated on step 1 passing)* — builds multi-arch (`amd64`/`arm64`) images and pushes to Docker Hub under both `:latest` and `:<commit-sha>`
